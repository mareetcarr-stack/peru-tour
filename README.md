...
- **Checklist items are editable in the app**: each day has a **Select all / Deselect all** button (one write for the whole day; clearing asks first) and an **Edit items** toggle that swaps that day's rows for text fields with a delete button, plus a field to add an item. Deleting blanks the item's row in `DAILY CHECKLIST` rather than removing it, so the row numbers of items below stay valid; adding reuses a blank row inside the day's block when there is one and only inserts a new sheet row when the block is full. Item text can't be saved empty — a blank column B is how a deleted item is stored, so an empty item would silently disappear.
